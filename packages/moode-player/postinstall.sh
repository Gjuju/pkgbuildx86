...
function on_upgrade() {
	...
	# Introduced in r1024
	dpkg --compare-versions $VERSION lt "10.2.4-1moode1"
	if [ $? -eq 0 ]; then
		echo "There are no postinstall updates for 10.2.4"
	...
	if [ $? -eq 0 ]; then
		...
		sqlite3 $SQLDB "CREATE TABLE cfg_rcucache (id INTEGER PRIMARY KEY, title CHAR (32), cover_url CHAR (32))"
		# Remove test iTunes cover search utility
